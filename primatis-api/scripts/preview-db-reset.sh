#!/usr/bin/env bash
#
# PRIMATIS — Reset de la base preview dédiée (DEV-16.8, rebuild final).
#
# Remet `primatis_preview` dans un état vide et propre : DROP SCHEMA public
# CASCADE + CREATE SCHEMA public. Flyway (via le prochain démarrage de
# l'application sous le profil "preview", ou via une exécution directe des
# migrations) reconstruit ensuite le schéma à l'identique de primatis_dev/
# primatis_test/primatis_e2e, puis le pipeline data-seeding (profil "full")
...
# Le rôle applicatif "primatis", propriétaire de primatis_preview, peut en
...
#
# Garde-fou : la cible est strictement littérale ("primatis_preview"),
...
readonly PREVIEW_DB_NAME="primatis_preview"
...
echo "Reset de la base preview dédiée : ${PREVIEW_DB_NAME} (hôte ${PGHOST_LOCAL}:${PGPORT_LOCAL}, rôle ${PGUSER_LOCAL})"
...
actual_db="$(psql -h "${PGHOST_LOCAL}" -p "${PGPORT_LOCAL}" -U "${PGUSER_LOCAL}" -d "${PREVIEW_DB_NAME}" -tAc "SELECT current_database();")"
if [[ "${actual_db}" != "${PREVIEW_DB_NAME}" ]]; then
  echo "ERREUR : connexion inattendue à '${actual_db}' au lieu de '${PREVIEW_DB_NAME}' — abandon." >&2
  exit 1
fi

psql -h "${PGHOST_LOCAL}" -p "${PGPORT_LOCAL}" -U "${PGUSER_LOCAL}" -d "${PREVIEW_DB_NAME}" -v ON_ERROR_STOP=1 <<SQL
DROP SCHEMA public CASCADE;
CREATE SCHEMA public;
SQL

echo "primatis_preview : schéma public vidé. Prochaine étape : Flyway (mvn -Dspring-boot.run.profiles=preview) reconstruit le schéma, puis data-seeding (profil full) recharge catalogue + scénarios."
